...
- Install `@opennextjs/cloudflare` and `wrangler`. Follow the adapter's current docs to create
  ...
  assets binding). Add scripts: `preview` (`opennextjs-cloudflare build && opennextjs-cloudflare preview`) and `deploy`.
...
- **Verify:** `pnpm preview` serves locally under workerd. `pnpm deploy` publishes to `*.workers.dev` and the page loads. `wrangler deployments list` shows the cron trigger.
...
- `src/lib/env.ts`: validate server env at import with `zod` (fail fast with a clear message). Client-safe vars are read directly via `process.env.NEXT_PUBLIC_*`.
...
### 6. Test tooling
- **Vitest** for unit/integration: `vitest.config.ts`, `pnpm test`. Integration tests read `SUPABASE_URL`/keys from `supabase status -o env`.
...
### 7. CI (GitHub Actions)
- `.github/workflows/ci.yml` on PR and push to `main`: checkout → pnpm install (cached) → lint → typecheck → `supabase start` (use `supabase/setup-cli` action) → `pnpm test` → install Playwright browsers → `pnpm e2e` → upload Playwright report on failure.
- `.github/workflows/deploy.yml` on push to `main` after CI: `pnpm deploy` with `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` repo secrets.
...
- **Verify:** Lighthouse "Installable" check passes in Chrome DevTools on the preview build.
